...
class Favorites extends Component{
  render(){
      ...
      switch (v.mediatype) {
        ...
        cont = <div className="thumbnail" key={i}>
                   ...
                   <div className="hover animated fadeIn">
                     ...
                     <div className="half">
                      <i className="fa fa-trash" aria-hidden="true" onClick={this.props.delete} data-id={v.show_id}></i>
                     </div>
                   </div>
                 </div>
          movie.push(cont)
    ...
    if(lengMov !== 0){
      movie.unshift(<span className="element" key='mov-cumber-favorites'>{"Movies ("+lengMov+")"}</span>)
    }
    const lengSer = series.length
    if(lengSer !== 0){
      series.unshift(<span className="element" key='tv-cumber-favorites'>{"TV shows ("+lengSer+")"}</span>)
    }
    return(
